...
with st.container(border=True):
    ...
    treatment_editor = st.data_editor(treatment_combined, num_rows="dynamic")
    ...
    if total_checked > 0:
        ...
        st.write("Please select at least one treatment to allocate demand.")
...
with st.container( border=True):
    ...
    new_total_revenue, new_total_cost, new_total_profit, updated_df = model_instance.calculate_existing_and_upsell(demand_df, upsell_configuration)

    with st.expander("New Treatment Demand", expanded =False):
