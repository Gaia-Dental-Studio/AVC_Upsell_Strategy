...
def app():
    ...
    with st.container(border=True):
        ...
        #     st.dataframe(demand_df, use_container_width=False, width=500, hide_index=True)
        # else:
        #     st.warning("Please select at least one treatment to allocate demand.")
            
        
        selected_clinic = st.selectbox("Select Clinic", ["Acre Clarity Dental"], key='selected_clinic', index=0)
        
        demand_df = pd.read_csv('clarity_treatment_1.csv')
        demand_df = demand_df[['Treatment', 'Demand']]
        
        
        pricing_basis = st.radio("Select Pricing Basis", ["Indonesia", 'Australia'], key='pricing_basis', index=1)
    ...
    if selected_clinic:
        ...
        with st.container(border=True):
            ...
            model_instance.pricing_basis_selection(pricing_basis)
            ...
            with col1:
                st.metric("Total Revenue", model_instance.format_currency(total_revenue, pricing_basis))
                st.metric("Total Cost", model_instance.format_currency(total_cost, pricing_basis))
                

            with col2:
                total_profit = total_revenue - total_cost
                st.metric("Total Profit", model_instance.format_currency(total_profit, pricing_basis))
            ...
            item_code_df = model_instance.analyze_item_code_performance(demand_df, pricing_basis)
            ...
            st.pyplot(model_instance.create_combo_chart(item_code_df, pricing_basis))
        ...
        with st.container( border=True):
            ...
            # Analyze item code performance using the demand DataFrame
            item_code_df = model_instance.analyze_item_code_performance(updated_df, pricing_basis)
